...
- Fuzzy search (`mode=fuzzy`) also searches cities and towns (Overture `locality` and `localadmin`), stored apart in `overture_normalized/localities/`. The natural-language mode never sees them, because its model was not trained on them.
